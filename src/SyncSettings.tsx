...
import type { DriveUploads, UploadItem } from "./useDriveUploads";
...
export default function SyncSettings({drive, uploads, folders, onClose, cloudLoading, cloudError, onRefreshCloud}: {
  ...
  async function resolve(root: string, item: UploadItem, restore: boolean) {
    if (resolving) return;
    setResolving(`${root}\n${item.path}`);
    setResolutionError("");
    try {
      if (!restore && !(await confirmCloudDeletion(item.path))) return;
      await uploads.resolveMissing(root, item, restore);
    } catch (error) { setResolutionError(String(error)); }
    finally { setResolving(null); }
  }
  const spaces=folders.filter(folder=>folder.cloudSpace);
  return <dialog ref={dialog} className="settings-dialog sync-dialog" aria-labelledby="sync-title" onCancel={event=>{event.preventDefault();if (!resolving) onClose();}}>
    <header className="settings-header"><div className="settings-emblem"><Cloud size={21} aria-hidden="true" /></div><div><h1 id="sync-title">Cloud</h1><p>Your notes, available on every connected device.</p></div><button autoFocus className="icon-button" aria-label="Close Cloud settings" disabled={!!resolving} onClick={onClose}><X size={18}/></button></header>
    <div className="sync-body"><div className="sync-intro">
      ...
      {drive.status.connected && <>
        ...
        <div className="cloud-spaces-heading"><h2>Your spaces</h2><button className="settings-action" disabled={!!resolving || cloudLoading || !!uploads.activeRoot} onClick={onRefreshCloud}><RefreshCw size={14} aria-hidden="true" />{cloudLoading ? "Refreshing…" : "Refresh Cloud"}</button></div>
        ...
        {spaces.map(space=><section className="cloud-space-card" key={space.root}><h2>{space.name}</h2><p>{space.files.length} notes · {uploads.activeRoot===space.root ? "Syncing…" : uploads.errors[space.root] || (uploads.completed[space.root] ? "Up to date" : "Stored on this device")}</p>
          {Object.entries(uploads.items).filter(([key,item])=>key.startsWith(`${space.root}\n`) && item.state === "error").map(([key,item])=><div className="cloud-file-issue" key={key}><p role="alert"><strong>{item.path}</strong>: {item.message}</p>
            {item.missingDriveId && <div className="cloud-space-actions" role="group" aria-label={`Resolve ${item.path}`}>
              <button className="settings-action" disabled={!!resolving || !!uploads.activeRoot || cloudLoading} title="Upload the saved local note as a new copy in Google Drive" onClick={()=>void resolve(space.root,item,true)}>Restore to Cloud</button>
              <button className="settings-action" disabled={!!resolving || !!uploads.activeRoot || cloudLoading} onClick={()=>void resolve(space.root,item,false)}>Acknowledge &amp; delete…</button>
              {resolving===key && <span role="status">Resolving…</span>}
            </div>}
          </div>)}
          ...
          <button className="settings-action" disabled={!!resolving || !!uploads.activeRoot || cloudLoading} onClick={()=>void uploads.upload(space.root)}><RefreshCw size={14} aria-hidden="true" />Retry sync</button></div>
        </section>)}
        {!spaces.length && !cloudLoading && !cloudError && <p className="cloud-empty">No Cloud spaces yet.</p>}
        </div>
      </>}
    </div></div><footer className="settings-footer"><small>Cloud storage uses Nova’s .nova folder in Google Drive.</small><button disabled={!!resolving} onClick={onClose}>Done</button></footer>
  </dialog>;
}
