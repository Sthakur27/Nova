...
3. **Choose your view.** For Markdown notes, **Edit** lets you write with formatting, **Source** shows the underlying text, and optional **Read** gives you a reading view. Enable **Settings → Show Read mode** to add Read; it defaults off on desktop and mobile. Large Markdown notes retain a reading fallback. In Edit, type `-` + Space for bullets or `1` + Space for numbering; [Markdown typing shortcuts](docs/user-guide.md#workspace-and-formatting) also cover headings, tasks, quotes, and inline formatting. Use **Code block** in the toolbar for multiline code, or type three backticks and Enter.
