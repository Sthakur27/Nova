...
use std::{io::{BufRead, BufReader, Write}, net::TcpListener, sync::{Arc, Mutex, atomic::{AtomicBool, Ordering}}, time::{Duration, Instant}};
use tauri::State;
...
pub struct Status { connected: bool, email: Option<String>, configured: bool }
...
fn connection_status(credential: Option<Credential>) -> Status {
    Status { connected: credential.is_some(), email: credential.map(|c| c.email), configured: secret().is_ok() }
}
fn status() -> Result<Status, String> { Ok(connection_status(cached_credential()?)) }
#[tauri::command]
...
#[tauri::command]
pub fn drive_cancel(auth: State<'_, DriveAuth>) { auth.cancelled.store(true, Ordering::SeqCst); }
struct ActiveGuard(Arc<AtomicBool>);
impl Drop for ActiveGuard { fn drop(&mut self) { self.0.store(false, Ordering::SeqCst); } }
#[tauri::command]
pub async fn drive_connect(auth: State<'_, DriveAuth>) -> Result<Status, String> {
    ...
    tauri::async_runtime::spawn_blocking(move || { let _guard = guard; connect(&cancelled) }).await
...
fn connect(cancelled: &AtomicBool) -> Result<Status, String> {
...
pub(crate) fn access_token() -> Result<String, String> {
    ...
    let response = client.post("https://oauth2.googleapis.com/token").form(&[
        ("client_id", CLIENT_ID), ("client_secret", secret()?), ("refresh_token", credential.refresh_token.as_str()), ("grant_type", "refresh_token"),
