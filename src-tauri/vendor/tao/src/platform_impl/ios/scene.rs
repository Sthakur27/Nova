...
// true when the system allows the app to display multiple scenes and scene_lifecycle_enabled() returns true
...
// Scene adoption is independent of whether the app allows multiple windows.
pub unsafe fn scene_lifecycle_enabled() -> bool {
  NSBundle::mainBundle()
    .infoDictionary()
    .is_some_and(|info| info.objectForKey(&NSString::from_str("UIApplicationSceneManifest")).is_some())
}
