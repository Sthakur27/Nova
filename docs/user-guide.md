...
- **Google Drive**: connect through **Settings → Google Drive → Set up sync** or the cloud button beside Settings. Connected desktop and iOS builds sync selected saved notes and can restore a Drive workspace onto another device. Checks run every minute while active and when returning to the foreground; iOS stores restored workspaces in private app storage. Explorer cloud buttons toggle inclusion; tab cloud buttons open settings for that file. See [Google Drive](sync.md) for setup, selection rules, upload status, and current limits.
