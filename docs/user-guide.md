...
- **Command-F / Ctrl-F**: toggle find within the note, with match counts and Enter / Shift-Enter navigation. Formatted Edit and Read keep the current view and scroll to each match. In Source, expand **Search options & replace** for case-sensitive, whole-word, or regex matching, selecting all matches, and replacing one or all matches. Escape also closes find; reopening retains the query while the editor stays mounted.
