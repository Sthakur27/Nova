...
- **Google Drive**: connect through **Settings → Google Drive → Set up sync** or the cloud button beside Settings. Connected desktop builds sync selected saved notes, check incoming changes every minute and on window focus, and can download a workspace onto another desktop. Explorer cloud buttons toggle inclusion; tab cloud buttons open settings for that file. See [Google Drive](sync.md) for setup, selection rules, upload status, and current limits.
...
- No general filesystem watcher, automatic reload of external local edits, image attachments, or plugin compatibility yet. Clean open notes do refresh after Drive downloads. Desktop Google Drive checks selected paths for incoming changes every minute and preserves both sides of conflicting edits. Deletions do not propagate, and automatic conflict merging is not available. The folder menu’s Refresh action and reopening a file pick up disk changes.
