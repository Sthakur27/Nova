...
- **Source / Edit / Read**: Source edits raw Markdown. For Markdown notes up to 500,000 characters, Edit and Read share the same document layout, including headings, formatted text, nested lists, tasks, quotes, code blocks, and tables. Edit adds a formatting toolbar and direct text editing; Read only allows task checkboxes to change. Tab/Shift-Tab nest and outdent list items. Switching modes does not rewrite the file; checkbox toggles change only the check character. Unsupported constructs remain intact and can be edited in Source. Larger Markdown notes use source editing and a paginated reader. Files selected in visible panes are loaded; other tabs retain their saved editor state.
