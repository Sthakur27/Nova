...
- **Sync selection**: open **Sync** in the sidebar or top tab bar, or use a note tab’s cloud button to focus that file. Switch folders and search paths in the dialog. Choices are saved locally for future sync; Google Drive is not connected and nothing is uploaded. See [sync selection](sync.md) for inheritance and exclusions.
