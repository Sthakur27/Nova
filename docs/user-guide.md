...
- **External Local changes**: on desktop, Nova checks metadata for open Local files and the root/expanded directories every three seconds while visible and when the window regains focus. Clean notes reload automatically; folder listings retain already loaded pages. Changed or deleted files with unsaved edits keep their open copy and recovery draft, with a notice. Saves still check the original disk revision. **Refresh folder** remains available for a manual reload of the root and expanded directories.
...
- External Local detection uses metadata polling, not a recursive filesystem watcher. It monitors open tabs and expanded directories only, pauses while the window is hidden, and can miss changes that preserve all observed metadata on filesystems with coarse timestamps. Manual Refresh and reopening remain available. No image attachments or plugin compatibility yet. Clean open notes also refresh after Drive downloads. Desktop and iOS Google Drive check selected paths for incoming changes every minute and preserve both sides of conflicting edits. Deletions do not propagate, and automatic conflict merging is not available.
